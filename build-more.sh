...
sudo /usr/local/tahti/venv/python setup.py install || { echo "PYINDI installation failed"; exit 1; }
...
sudo /usr/local/tahti/venv/bin/pip install git+https://github.com/knro/indiwebmanager python-pam six flask_cors 
sudo /usr/local/tahti/venv/bin/pip install --use-pep517 git+https://github.com/joxda/gps3.git
